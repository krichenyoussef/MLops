...
# --------------------------------------------------
# Train CatBoost with params.yaml config
# --------------------------------------------------
for fold, (tr_idx, val_idx) in enumerate(folds):
    print(f"\n🔹 Training fold {fold + 1}/{TRAIN_CFG['n_splits']}")

    model = CatBoostClassifier(**CAT_CFG)
    ...
    model.fit(
        ...
        verbose=CAT_CFG.get("verbose", False),
    )
    ...
    # Save model
    # --------------------------------------------------
    model_path = MODELS_DIR / f"catboost_fold_{fold}.pkl"
